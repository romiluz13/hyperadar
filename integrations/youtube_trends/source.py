        ...
        try:
            ...
            for line in output.split("\n"):
                ...
                if "youtube.com/watch" in url or "youtu.be" in url:
                    ...
                    clean_url = (
                        f"https://www.youtube.com/watch?v={vid_match.group(1)}"
                        if vid_match
                        else url
                    )
                    candidates.append(
                        {
                            ...
                            "title": title[:200],
                            "kind": "video",
                            "description": parts[3].strip()[:300]
                            if len(parts) > 3
                            else title,
                            "topics": ["youtube", "ai", "video"],
                            "serp_rank": int(parts[0].strip().split()[0])
                            if parts[0].strip()
                            else 99,
                            "stars": max(
                                100 - int(parts[0].strip().split()[0] or 99), 10
                            ),
                        }
                    )
