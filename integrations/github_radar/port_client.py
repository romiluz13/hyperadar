...
def upsert_agent(handle: str, name: str, bio: str, source_type: str,
                 run_count: int = 0, success_rate: float = 100.0) -> dict:
    """Upsert the AgentCreator entity. Identifier is a slug; @handle is a property."""
    ident = _slug(handle)
    return _upsert("hyperadar_agent", ident, {
        "title": name,
        "properties": {
            "handle": handle, "name": name, "bio": bio,
            "sourceType": source_type, "status": "active",
            "lastRunAt": _iso_now(), "runCount": run_count, "successRate": success_rate,
        },
    })


def upsert_project(url: str, title: str, kind: str, description: str,
                    topics: list[str], momentum_score: float, hype_verdict: str) -> dict:
    """Upsert a Project entity (identifier = slugified url)."""
    ident = _slug(url)
    return _upsert("hyperadar_project", ident, {
        "title": title,
        "properties": {
            "title": title, "url": url, "kind": kind,
            "description": description, "topics": topics,
            "momentumScore": momentum_score, "hypeVerdict": hype_verdict,
            "lastSeenAt": _iso_now(),
        },
    })


def upsert_post(post_id: str, agent_handle: str, project_url: str,
                 body: str, verdict: str, rank_score: float,
                 like_count: int = 0, comment_count: int = 0, share_count: int = 0) -> dict:
    """Upsert a Post entity with relations to agent + project."""
    project_ident = _slug(project_url)
    agent_ident = _slug(agent_handle)
    return _upsert("hyperadar_post", post_id, {
        "title": f"{agent_handle} post",
        "properties": {
            "body": body, "verdict": verdict, "rankScore": rank_score,
            "likeCount": like_count, "commentCount": comment_count, "shareCount": share_count,
            "postedAt": _iso_now(),
        },
